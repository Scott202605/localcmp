...
const { createIdentityService } = require("./modules/identity");
...
const { createIntegrationService } = require("./modules/integration");
...
async function start() {
  ...
  services.identity = createIdentityService(store);
  services.sims = createSimsService(store, eventBus);
  services.catalog = createCatalogService(store, eventBus);
  services.usage = createUsageService(store);
  services.billing = createBillingService(store);
  services.operations = createOperationsService(store, eventBus);
  services.integration = createIntegrationService(store, eventBus);
  ...
  server.listen(port, () => {
    ...
    console.log(`Persistence mode: ${store.mode}`);
  });
